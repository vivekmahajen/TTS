...
def optimize_for_speech(content, api_key, progress_callback=None):
    ...
    try:
        ...
        if cached_content:
            ...
            st.info("\U0001f680 Using cached optimized content (no API call needed)")
...
"""
        ...
        if save_optimized_content(content_hash, optimized_content):
            st.success("\U0001f4be Optimized content saved to cache for future use!")
...
def main():
    ...
    st.title("\U0001f3b5 Markdown to Speech Converter")
    ...
    with st.sidebar:
        ...
        st.subheader("\U0001f916 AI Optimization (Optional)")
        ...
        if OPENAI_AVAILABLE:
            ...
            with col2:
                if st.button("\U0001f4be", help="Save API key"):
                    if api_key.strip():
            ...
            if stored_api_key:
                if st.button("\U0001f5d1️ Delete Stored Key", help="Remove saved API key"):
            ...
            if stored_api_key:
                st.info("\U0001f510 API key loaded from storage")
            ...
            if use_openai or api_key:
                st.markdown("---")
                st.subheader("\U0001f4e6 Optimization Cache")
                ...
                if cache_count > 0:
                    st.info(f"\U0001f4be Cached items: {cache_count} ({cache_size_mb:.1f} MB)")
                    
                    col1, col2 = st.columns(2)
                    with col1:
                        if st.button("\U0001f5d1️ Clear Cache", help="Remove all cached optimizations"):
                    ...
                    with col2:
                        if st.button("\U0001f4ca Cache Info", help="Show cache details"):
                    ...
                    st.info("\U0001f4be No cached optimizations yet")
        else:
        ...
        st.subheader("\U0001f6ab Exclude Signs")
        signs_to_exclude = []
    ...
    if proceed_buttons_clicked:
        ...
        latest_proceed_key = proceed_buttons_clicked[-1]
    ...
    if getattr(st.session_state, 'should_convert', False):
        ...
        st.success(f"\U0001f680 Ready to convert! Using content: {len(content)} characters")
        
        base_name = os.path.splitext(filename)[0]
        output_file = f"{base_name}.mp3"
        logger.info(f"Output file will be: {output_file}")
        
        st.info(f"\U0001f504 Starting conversion of {len(content)} characters to {output_file}")
        st.write(f"\U0001f50d **Debug:** About to call markdown_to_speech with:")
        ...
        st.subheader("\U0001f504 Conversion Progress")
        ...
        if success and os.path.exists(output_file):
            ...
            st.success("\U0001f389 Conversion completed successfully!")
            st.write(f"\U0001f50d **Debug:** File created successfully! Size: {file_size} bytes")
            
            with open(output_file, "rb") as file:
                st.download_button(
                    label="\U0001f4e5 Download Audio File",
                    data=file.read(),
            ...
            st.write("\U0001f50d **Debug:** Conversion failed - check the logs above for details")
    ...
    st.header("\U0001f4c4 Upload Markdown File or Enter Content")
    ...
    with col2:
            if st.button("\U0001f3b5 Convert to Speech", type="primary", use_container_width=True):
                ...
                if content:
                    ...
                    st.write(f"\U0001f50d **Debug:** Starting conversion of {len(content)} characters")
                    ...
                    if use_openai and api_key:
                        ...
                        optimized_content_key = f"optimized_content_{content_hash}"
                        ...
                        if already_optimized:
                            ...
                            st.subheader("\U0001f916 AI Optimization")
                            st.write("\U0001f50d **Debug:** AI optimization enabled, starting process...")
                        ...
                        st.subheader("\U0001f4dd Content Comparison")
                        ...
                        with st.expander("\U0001f4d6 View Full Optimized Content"):
                            st.text_area("Complete Optimized Content", value=content, height=400, disabled=True, key="full_optimized")
                        
                        st.info("\U0001f4cb Review the optimized content above. Click 'Proceed with Conversion' to continue or edit the content manually below.")
                        ...
                        if content:
                            st.info(f"\U0001f4ca Content ready for conversion: {len(content)} characters")
                        else:
                        ...
                        proceed_button_clicked = st.button("\U0001f3b5 Proceed with Conversion", type="secondary", use_container_width=True, key=proceed_key)
                        ...
                        st.info("\U0001f446 Review the content above, then click 'Proceed with Conversion' to continue.")
